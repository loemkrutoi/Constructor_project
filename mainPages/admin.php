...
<body>
    ...
    if ((!empty($_POST['page']) && !(empty($_POST['pageKey'])) && isset($_POST))) {
        ...
        $pageId = $_POST["pageId"];
        $resultCheckExistance = Functions::query("SELECT * FROM `pages` WHERE `id_page` = '$pageId'");

        if($resultCheckExistance->num_rows>0){
            $resultUpdate = Functions::query("UPDATE `pages` SET `name_page` = '$page', `key_page` = '$pageKey' WHERE `id_page` = '$pageId'");
            rename('../secondarypages/' . $oldKey, '../secondarypages/' . $pageKey);
        }
        else if(!file_exists('../secondarypages/' . $pageKey)){
            $result = Functions::query("INSERT INTO `pages` (`name_page`, `key_page`) VALUES ('$page' , '$pageKey')");
    ...
    ?>
        <div id="insertAny" class="container col-12 vh-100 my-auto">
            ...
            <div class="container my-container vh-50 d-flex justify-content-between wrap-nowrap">
                <div class="left col-4 p-0">
                    <div class="top text-center">
                        <p>Все пункты меню</p>
                    </div>
                    <div ondragover="true" class="pagesList d-flex flex-nowrap flex-column bottom p-2 w-100 mx-auto">
                        <?php 
